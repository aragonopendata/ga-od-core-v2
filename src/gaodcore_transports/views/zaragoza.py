...
async def _download_processor_item(
    session: aiohttp.ClientSession, url: str, root_name: str, extra_data
) -> List[Dict[str, Any]]:
    ...
    config = [
        DownloadProcessorConfig(
            url=CONFIG.projects.transport.zaragoza.get_url(
                "destinations", id=origin_id
            ),
            root_name="destinations",
            extra_data={"origin_id": origin_id},
        )
        for origin_id in origins_ids
    ]
...
def _get_routes():
    ...
    configs = [
        DownloadProcessorConfig(
            url=CONFIG.projects.transport.zaragoza.get_url("routes", id=line["id"]),
            root_name="routes",
            extra_data={"line_id": line["id"]},
        )
...
class ArrivalTimeView(ZaragozaTransportMixin):  # pylint: disable=too-few-public-methods
    ...
    def get(self, _: Request, **_kwargs):
        ...
        configs = [
            DownloadProcessorConfig(
                url=CONFIG.projects.transport.zaragoza.get_url(
                    self._ENDPOINT, stop_id=stop[self._STOP_ID_FIELD]
                ),
                root_name=self._ENDPOINT,
...
class NoticesView(APIViewMixin):  # pylint: disable=too-few-public-methods
    ...
    def get(self, _: Request, **_kwargs):
        """Implementation of get of APIViewMixin."""
        data = download(CONFIG.projects.transport.zaragoza.get_url(self._ENDPOINT))[
            self._ENDPOINT
        ]
        if not data:
            data = []
...
class TimesRouteView(ZaragozaTransportMixin):  # pylint: disable=too-few-public-methods
    ...
    def get(self, _: Request, **_kwargs):
        """Implementation of get of APIViewMixin."""
        time_sec = self.request.query_params.get(
            self._TIME_SEC_PARAM, self._DEFAULT_DEPARTURE_TIME
        )
        configs = [
            DownloadProcessorConfig(
                url=CONFIG.projects.transport.zaragoza.get_url(
