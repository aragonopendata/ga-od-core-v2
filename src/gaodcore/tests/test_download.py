...
def test_download_view_id(endpoint: str, client: Client, full_example):
    download_response = client.get(endpoint, {
        'view_id': full_example.resources.table.id,
        "fields": ["id"]
    })
...
def test_download_non_existent_field_error(endpoint: str, accept_error, client: Client,
                                           full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
...
@pytest.mark.django_db
def test_download_format(endpoint: str, client: Client, full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        'formato': "csv"
    })

    with open(os.path.join(os.path.dirname(__file__), f"download.csv"), r'rb') as f:
        assert f.read() == download_response.content


@pytest.mark.django_db
def test_download_format_error(endpoint: str, client: Client,
                               full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        'formato': "dj"
...
def test_download_offset(endpoint: str, client: Client, full_example):
    ...
    with open(os.path.join(os.path.dirname(__file__), f"download.json"), r'rb') as f:
        assert json.loads(f.read())[1:] == json.loads(download_response.content)


@pytest.mark.django_db
def test_download_offset_error(endpoint: str, accept_error, client: Client,
                               full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        'offset': "a"
...
@pytest.mark.django_db
def test_download_limit_error(endpoint: str, accept_error, client: Client,
                              full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        'limit': "a"
...
@pytest.mark.django_db
def test_download_pagination_overflow(endpoint: str, client: Client,
                                      full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        '_page': "2",
...
@pytest.mark.django_db
def test_download_pagination_page_error(endpoint: str, accept_error, client: Client,
                                        full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        '_page': "a",
...
@pytest.mark.django_db
def test_download_pagination_page_size_error(endpoint: str, accept_error, client: Client,
                                             full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        '_page': "1",
...
def test_download_filters_value_error(endpoint: str, accept_error, client: Client):
    download_response = client.get(endpoint,
                                   {'resource_id': 1, 'filters': '{"a": []}'}, HTTP_ACCEPT=accept_error)
    assert download_response.status_code == 400
...
@pytest.mark.django_db
def test_download_sort_non_existent_field_error(endpoint: str, accept_error, client: Client,
                                                full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
...
@pytest.mark.django_db
def test_download_sort_mode_error(endpoint: str, accept_error, client: Client,
                                  full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        'sort': 'name none'
...
@pytest.mark.django_db
def test_download_sort_too_many_arguments_error(endpoint: str, accept_error, client: Client,
                                                full_example):
    download_response = client.get(endpoint, {
        'resource_id': full_example.resources.table.id,
        'sort': 'name none asd'
    ...
    assert download_response.status_code == 400
    validate_error(download_response.content,
                   'Sort value name none asd is not allowed. Too many arguments.', accept_error)
...
def test_download_resource_not_exists(endpoint: str, accept_error, client: Client):
    ...
    assert download_response.status_code == 400
    validate_error(download_response.content,
                   'Resource not exists or is not available', accept_error)


@pytest.mark.django_db
def test_download_extension(endpoint: str, accept_download: str, client: Client,
                            full_example):
    download_response = client.get(f'/GA_OD_Core/download',
                                   {'resource_id': full_example.resources.table.id},
                                   HTTP_ACCEPT=accept_download)
    compare_files(os.path.dirname(__file__), 'download', accept_download, download_response.content)
