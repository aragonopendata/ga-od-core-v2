...
def test_resources(client: Client, full_example):
    ...
    if full_example.scheme in ['http', 'https']:
        assert download_response.json() == [{'available': True, 'id': test_resources_id, 'name': 'test_resources'}]
    else:
        test_resources_view_id = ResourceConfig.objects.get(name='test_resources_view').id
