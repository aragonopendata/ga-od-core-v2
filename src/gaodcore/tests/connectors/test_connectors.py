...
def car_table():
    Car.create_table()
    Car.add_car("Model S", "Tesla", 2020, date(2020, 7, 13))
    Car.add_car("Model 3", "Tesla", 2021, date(2021, 4, 28))
    Car.add_car("Corsa", "Opel", 2019, date(2019, 2, 21))
    Car.add_car("Astra", "Opel", 2018, date(2018, 11, 15))
    Car.add_car("Clio", "Renault", 2017, date(2017, 8, 10))
    yield
    Car.delete_all()
...
def test_get_resource_gte_int(configs, car_table):
    ...
    assert len(result) == 2
    assert [r["name"] for r in result] == ["Model 3", "Model S"]
...
def test_get_resource_lte_int(configs, car_table):
    ...
    assert len(result) == 2
    assert [r["name"] for r in result] == ["Clio", "Astra"]
