...
@pytest.mark.django_db
def test_validator_invalid_location_schema_error(
...
@pytest.mark.django_db
def test_validator_config_path_error(
    auth_client: Client, connector_uri: str, accept_error: str
):
...
@pytest.mark.django_db
def test_validator_credentials_error(
    auth_client: Client,
    connector_uri: str,
...
def test_api_content_type_error(auth_client, httpserver: HTTPServer, accept_error):
    ...
    with open(path, "rb") as file:
        httpserver.expect_request(url).respond_with_data(
            file.read(), content_type="application/xlsx"
        )
