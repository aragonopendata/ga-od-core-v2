...
def test_validator(auth_client: Client, full_example: ConnectorData, accept_download: str):
    ...
    download_response = auth_client.get(f'/GA_OD_Core_admin/manager/validator', conf, HTTP_ACCEPT=accept_download)
...
def test_validator_invalid_uri_error(auth_client, accept_error):
    response = auth_client.get(f'/GA_OD_Core_admin/manager/validator', {
...
def test_validator_config_path_error(auth_client: Client, connector_uri: str, accept_error: str):
    ...
    download_response = auth_client.get(f'/GA_OD_Core_admin/manager/validator', data, HTTP_ACCEPT=accept_error)
...
def test_validator_too_many_rows(auth_client, full_example, mocker, accept_error):
    ...
    download_response = auth_client.get(f'/GA_OD_Core_admin/manager/validator', conf, HTTP_ACCEPT=accept_error)
    assert download_response.status_code == 400
...
def test_api_content_type_error(auth_client, httpserver: HTTPServer, accept_error):
    ...
    with open(path, 'rb') as f:
        httpserver.expect_request(url).respond_with_data(f.read(), content_type='application/xlsx')

    response = auth_client.get(f'/GA_OD_Core_admin/manager/validator', {'uri': httpserver.url_for(url)},
                               HTTP_ACCEPT=accept_error)
