...
class ResourceHealthCheckResultAdmin(admin.ModelAdmin):
    list_display = ['resource', 'check_time', 'health_status', 'response_time_ms', 'error_type_display']
    list_filter = ['is_healthy', 'error_type', 'check_time', 'resource']
    search_fields = ['resource__name', 'error_message']
    readonly_fields = ['resource', 'check_time', 'is_healthy', 'response_time_ms', 'error_message', 'error_type']
    ordering = ['-check_time']
    date_hierarchy = 'check_time'
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('resource', 'resource__connector_config')


@admin.register(ResourceHealthCheckAlert)
class ResourceHealthCheckAlertAdmin(admin.ModelAdmin):
    list_display = ['resource', 'alert_type', 'is_active', 'threshold_minutes', 'last_alert_time']
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('resource', 'alert_type', 'is_active')
        }),
        ('Alert Configuration', {
            'fields': ('threshold_minutes', 'consecutive_failures_threshold')
        }),
...
def run_resource_health_check(modeladmin, request, queryset):
    ...
    for resource in queryset:
        try:
            ...
            results.append(f"✓ {resource.name}: {'Healthy' if result.is_healthy else 'Unhealthy'}")
        except Exception as e:
            results.append(f"✗ {resource.name}: Error - {str(e)}")

    modeladmin.message_user(request, "Resource health check results:\n" + "\n".join(results))
...
try:
    ...
    if hasattr(ResourceConfigAdmin, 'actions'):
        if isinstance(ResourceConfigAdmin.actions, tuple):
            ResourceConfigAdmin.actions = list(ResourceConfigAdmin.actions) + [run_resource_health_check]
        else:
