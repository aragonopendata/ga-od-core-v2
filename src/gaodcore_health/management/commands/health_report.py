...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--connector-id',
            type=int,
            help='Generate report for specific connector'
        )
        parser.add_argument(
            '--hours',
            type=int,
        ...
        parser.add_argument(
            '--days',
            type=int,
            help='Days to look back (overrides --hours)'
        )
        parser.add_argument(
            '--json',
            action='store_true',
        ...
        parser.add_argument(
            '--detailed',
            action='store_true',
            help='Include detailed error information'
        )
    ...
    def output_csv(self, summary: dict):
        ...
        self.stdout.write("connector_name,connector_id,total_checks,healthy_checks,unhealthy_checks,success_rate,avg_response_time_ms,is_currently_healthy,latest_check")
    ...
    def output_text(self, summary: dict, detailed: bool = False):
        ...
        for name, data in summary['connectors'].items():
            ...
            status_color = self.style.SUCCESS if data['is_currently_healthy'] else self.style.ERROR
    ...
    def output_detailed_errors(self, connector_id: int):
        ...
        if recent_errors.exists():
            ...
            for error in recent_errors:
                self.stdout.write(f"    {error.check_time}: {error.error_type} - {error.error_message}")
    ...
    def generate_weekly_report(self):
        ...
        for i in range(7):
            ...
            results = HealthCheckResult.objects.filter(
                check_time__gte=day_start,
                check_time__lt=day_end
            )
            ...
            if results.exists():
                ...
                self.stdout.write(f"{day_start.strftime('%Y-%m-%d')}: {success_rate:.1f}% ({healthy_count}/{total_count})")
            else:
    ...
    def generate_connector_trend(self, connector_id: int, days: int = 7):
        ...
        for i in range(days):
            ...
            if results.exists():
                ...
                self.stdout.write(f"{day_start.strftime('%Y-%m-%d')}: {status} {success_rate:.1f}% (avg: {avg_response_str})")
            else:
                self.stdout.write(f"{day_start.strftime('%Y-%m-%d')}: No data")
